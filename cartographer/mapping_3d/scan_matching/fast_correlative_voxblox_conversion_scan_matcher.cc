...
CreateFastCorrelativeVoxbloxConversionScanMatcherOptions(
...
int ComputeVoxelWidth(const std::shared_ptr<voxblox::TsdfMap> hybrid_grid){
    ...
    for (const voxblox::BlockIndex& index : blocks) {
      ...
      for (size_t linear_index = 0; linear_index < num_voxels_per_block;
        ...
        bounding_box_max = bounding_box_max.cwiseMax(coord);
        bounding_box_min = bounding_box_max.cwiseMax(coord);
      }
    }

    Eigen::Vector3f volume_size = bounding_box_max - bounding_box_min;
...
FastCorrelativeVoxbloxConversionScanMatcher::FastCorrelativeVoxbloxConversionScanMatcher(const std::shared_ptr<voxblox::TsdfMap> hybrid_grid,
    ...
    : options_(options),
      ...
      origin_({0,0,0}),//todo(kdaun) add origin to voxblox_map
...
FastCorrelativeVoxbloxConversionScanMatcher::~FastCorrelativeVoxbloxConversionScanMatcher() {}

bool FastCorrelativeVoxbloxConversionScanMatcher::Match(
...
bool FastCorrelativeVoxbloxConversionScanMatcher::MatchFullSubmap(
...
bool FastCorrelativeVoxbloxConversionScanMatcher::MatchWithSearchParameters(
    const FastCorrelativeVoxbloxConversionScanMatcher::SearchParameters& search_parameters,
    const transform::Rigid3d& initial_pose_estimate,
    const sensor::PointCloud& coarse_point_cloud,
    const sensor::PointCloud& fine_point_cloud, const float min_score,
    float* const score, transform::Rigid3d* const pose_estimate) const {
...
DiscreteScan FastCorrelativeVoxbloxConversionScanMatcher::DiscretizeScan(
    const FastCorrelativeVoxbloxConversionScanMatcher::SearchParameters& search_parameters,
...
std::vector<DiscreteScan> FastCorrelativeVoxbloxConversionScanMatcher::GenerateDiscreteScans(
    const FastCorrelativeVoxbloxConversionScanMatcher::SearchParameters& search_parameters,
    const sensor::PointCloud& coarse_point_cloud,
...
FastCorrelativeVoxbloxConversionScanMatcher::GenerateLowestResolutionCandidates(
    const FastCorrelativeVoxbloxConversionScanMatcher::SearchParameters& search_parameters,
...
void FastCorrelativeVoxbloxConversionScanMatcher::ScoreCandidates(
...
std::vector<Candidate>
FastCorrelativeVoxbloxConversionScanMatcher::ComputeLowestResolutionCandidates(
    const FastCorrelativeVoxbloxConversionScanMatcher::SearchParameters& search_parameters,
...
Candidate FastCorrelativeVoxbloxConversionScanMatcher::BranchAndBound(
    const FastCorrelativeVoxbloxConversionScanMatcher::SearchParameters& search_parameters,
