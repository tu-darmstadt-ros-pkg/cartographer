...
SparsePoseGraphVoxbloxESDFConversion::SparsePoseGraphVoxbloxESDFConversion(
...
void SparsePoseGraphVoxbloxESDFConversion::GrowSubmapTransformsAsNeeded(
    const std::vector<const VoxbloxESDF*>& insertion_submaps) {
...
void SparsePoseGraphVoxbloxESDFConversion::AddScan(
      common::Time time,
      const sensor::RangeData& range_data_in_tracking,
      const transform::Rigid3d& pose,
      const kalman_filter::PoseCovariance& covariance,
      int trajectory_id, const VoxbloxESDF* matching_submap,
      const std::vector<const VoxbloxESDF*>& insertion_submaps) {
    ...
    const VoxbloxESDF* finished_submap =
...
void SparsePoseGraphVoxbloxESDFConversion::AddWorkItem(std::function<void()> work_item) {
...
void SparsePoseGraphVoxbloxESDFConversion::AddImuData(const int trajectory_id, common::Time time,
...
void SparsePoseGraphVoxbloxESDFConversion::ComputeConstraint(const mapping::NodeId& node_id,
                                                  const mapping::SubmapId& submap_id) {
...
void SparsePoseGraphVoxbloxESDFConversion::ComputeConstraintsForOldScans(const VoxbloxESDF* submap) {
...
void SparsePoseGraphVoxbloxESDFConversion::ComputeConstraintsForScan(
        const VoxbloxESDF* matching_submap,
        std::vector<const VoxbloxESDF*> insertion_submaps,
        const VoxbloxESDF* finished_submap, const transform::Rigid3d& pose,
        const kalman_filter::PoseCovariance& covariance) {
    ...
    for (const VoxbloxESDF* submap : insertion_submaps) {
...
void SparsePoseGraphVoxbloxESDFConversion::HandleScanQueue() {
constraint_builder_.WhenDone(
    [this](const sparse_pose_graph::ConstraintBuilderVoxbloxESDFConversion::Result& result) {
      {
        common::MutexLocker locker(&mutex_);
...
void SparsePoseGraphVoxbloxESDFConversion::WaitForAllComputations() {
    ...
    constraint_builder_.WhenDone(
        [this, &notification](
            const sparse_pose_graph::ConstraintBuilderVoxbloxESDFConversion::Result& result) {
          common::MutexLocker locker(&mutex_);
          constraints_.insert(constraints_.end(), result.begin(), result.end());
          notification = true;
        });
...
void SparsePoseGraphVoxbloxESDFConversion::RunFinalOptimization() {
    WaitForAllComputations();
...
void SparsePoseGraphVoxbloxESDFConversion::RunOptimization() {
...
SparsePoseGraphVoxbloxESDFConversion::GetTrajectoryNodes() {
  common::MutexLocker locker(&mutex_);
  return trajectory_nodes_.data();
}

std::vector<SparsePoseGraphVoxbloxESDFConversion::Constraint> SparsePoseGraphVoxbloxESDFConversion::constraints() {
  common::MutexLocker locker(&mutex_);
  return constraints_;
}

transform::Rigid3d SparsePoseGraphVoxbloxESDFConversion::GetLocalToGlobalTransform(
...
std::vector<std::vector<int>> SparsePoseGraphVoxbloxESDFConversion::GetConnectedTrajectories() {
  common::MutexLocker locker(&mutex_);
  return connected_components_;
}

std::vector<transform::Rigid3d> SparsePoseGraphVoxbloxESDFConversion::GetSubmapTransforms(
    const int trajectory_id) {
  common::MutexLocker locker(&mutex_);
...
std::vector<transform::Rigid3d> SparsePoseGraphVoxbloxESDFConversion::ExtrapolateSubmapTransforms(
