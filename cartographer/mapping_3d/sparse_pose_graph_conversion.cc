...
SparsePoseGraphConversion::SparsePoseGraphConversion(
...
void SparsePoseGraphConversion::GrowSubmapTransformsAsNeeded(
...
int SparsePoseGraphConversion::AddScan(
...
void SparsePoseGraphConversion::AddWorkItem(std::function<void()> work_item) {
...
void SparsePoseGraphConversion::AddImuData(const mapping::Submaps* trajectory,
                                 common::Time time,
...
void SparsePoseGraphConversion::ComputeConstraint(const int scan_index,
...
void SparsePoseGraphConversion::ComputeConstraintsForOldScans(const TSDF* submap) {
...
void SparsePoseGraphConversion::ComputeConstraintsForScan(
...
void SparsePoseGraphConversion::HandleScanQueue() {
  constraint_builder_.WhenDone(
      [this](const sparse_pose_graph::ConstraintBuilderConversion::Result& result) {
...
void SparsePoseGraphConversion::WaitForAllComputations() {
  ...
  constraint_builder_.WhenDone(
      [this, &notification](
          const sparse_pose_graph::ConstraintBuilderConversion::Result& result) {
        constraints_.insert(constraints_.end(), result.begin(), result.end());
        common::MutexLocker locker(&mutex_);
        notification = true;
      });
...
void SparsePoseGraphConversion::RunFinalOptimization() {
  WaitForAllComputations();
...
void SparsePoseGraphConversion::RunOptimization() {
...
bool SparsePoseGraphConversion::HasNewOptimizedPoses() {
  common::MutexLocker locker(&mutex_);
...
SparsePoseGraphConversion::GetScanMatchingProgress() {
...
std::vector<mapping::TrajectoryNode> SparsePoseGraphConversion::GetTrajectoryNodes() {
  common::MutexLocker locker(&mutex_);
  return trajectory_nodes_;
}

std::vector<SparsePoseGraphConversion::Constraint> SparsePoseGraphConversion::constraints() {
  return constraints_;
}

transform::Rigid3d SparsePoseGraphConversion::GetLocalToGlobalTransform(
...
SparsePoseGraphConversion::GetConnectedTrajectories() {
  common::MutexLocker locker(&mutex_);
  return connected_components_;
}

std::vector<transform::Rigid3d> SparsePoseGraphConversion::GetSubmapTransforms(
    const mapping::Submaps& submaps) {
  common::MutexLocker locker(&mutex_);
  return ExtrapolateSubmapTransforms(optimized_submap_transforms_, submaps);
}

std::vector<transform::Rigid3d> SparsePoseGraphConversion::ExtrapolateSubmapTransforms(
