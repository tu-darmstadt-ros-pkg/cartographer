...
SparsePoseGraphVoxbloxConversion::SparsePoseGraphVoxbloxConversion(
...
void SparsePoseGraphVoxbloxConversion::GrowSubmapTransformsAsNeeded(
    const std::vector<const VoxbloxTSDF*>& insertion_submaps) {
...
void SparsePoseGraphVoxbloxConversion::AddScan(
      common::Time time,
      const sensor::RangeData& range_data_in_tracking,
      const transform::Rigid3d& pose,
      const kalman_filter::PoseCovariance& covariance,
      int trajectory_id, const VoxbloxTSDF* matching_submap,
      const std::vector<const VoxbloxTSDF*>& insertion_submaps) {
    ...
    const VoxbloxTSDF* finished_submap =
...
void SparsePoseGraphVoxbloxConversion::AddWorkItem(std::function<void()> work_item) {
...
void SparsePoseGraphVoxbloxConversion::AddImuData(const int trajectory_id, common::Time time,
...
void SparsePoseGraphVoxbloxConversion::ComputeConstraint(const mapping::NodeId& node_id,
                                                  const mapping::SubmapId& submap_id) {
...
void SparsePoseGraphVoxbloxConversion::ComputeConstraintsForOldScans(const VoxbloxTSDF* submap) {
...
void SparsePoseGraphVoxbloxConversion::ComputeConstraintsForScan(
        const VoxbloxTSDF* matching_submap,
        std::vector<const VoxbloxTSDF*> insertion_submaps,
        const VoxbloxTSDF* finished_submap, const transform::Rigid3d& pose,
        const kalman_filter::PoseCovariance& covariance) {
    ...
    for (const VoxbloxTSDF* submap : insertion_submaps) {
...
void SparsePoseGraphVoxbloxConversion::HandleScanQueue() {
constraint_builder_.WhenDone(
    [this](const sparse_pose_graph::ConstraintBuilderVoxbloxConversion::Result& result) {
      {
        common::MutexLocker locker(&mutex_);
...
void SparsePoseGraphVoxbloxConversion::WaitForAllComputations() {
    ...
    constraint_builder_.WhenDone(
        [this, &notification](
            const sparse_pose_graph::ConstraintBuilderVoxbloxConversion::Result& result) {
          common::MutexLocker locker(&mutex_);
          constraints_.insert(constraints_.end(), result.begin(), result.end());
          notification = true;
        });
...
void SparsePoseGraphVoxbloxConversion::RunFinalOptimization() {
    WaitForAllComputations();
...
void SparsePoseGraphVoxbloxConversion::RunOptimization() {
...
SparsePoseGraphVoxbloxConversion::GetTrajectoryNodes() {
  common::MutexLocker locker(&mutex_);
  return trajectory_nodes_.data();
}

std::vector<SparsePoseGraphVoxbloxConversion::Constraint> SparsePoseGraphVoxbloxConversion::constraints() {
  common::MutexLocker locker(&mutex_);
  return constraints_;
}

transform::Rigid3d SparsePoseGraphVoxbloxConversion::GetLocalToGlobalTransform(
...
std::vector<std::vector<int>> SparsePoseGraphVoxbloxConversion::GetConnectedTrajectories() {
  common::MutexLocker locker(&mutex_);
  return connected_components_;
}

std::vector<transform::Rigid3d> SparsePoseGraphVoxbloxConversion::GetSubmapTransforms(
    const int trajectory_id) {
  common::MutexLocker locker(&mutex_);
...
std::vector<transform::Rigid3d> SparsePoseGraphVoxbloxConversion::ExtrapolateSubmapTransforms(
