...
#ifndef CARTOGRAPHER_MAPPING_3D_SPARSE_POSE_GRAPH_CONSTRAINT_BUILDER_VOXBLOX_CONVERSION_H_
#define CARTOGRAPHER_MAPPING_3D_SPARSE_POSE_GRAPH_CONSTRAINT_BUILDER_VOXBLOX_CONVERSION_H_
...
class ConstraintBuilderVoxbloxConversion {
 public:
  using Constraint = mapping::SparsePoseGraph::Constraint;
  using Result = std::vector<Constraint>;

  ConstraintBuilderVoxbloxConversion(
      const mapping::sparse_pose_graph::proto::ConstraintBuilderOptions&
          options,
      common::ThreadPool* thread_pool);
  ~ConstraintBuilderVoxbloxConversion();

  ConstraintBuilderVoxbloxConversion(const ConstraintBuilderVoxbloxConversion&) = delete;
  ConstraintBuilderVoxbloxConversion& operator=(const ConstraintBuilderVoxbloxConversion&) = delete;
  ...
  void MaybeAddConstraint(const mapping::SubmapId& submap_id, const VoxbloxTSDF* const submap,
  ...
  void MaybeAddGlobalConstraint(
      const mapping::SubmapId& submap_id, const VoxbloxTSDF* const submap,
 ...
 private:
  ...
  void ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(
      ...
      const std::vector<mapping::TrajectoryNode>& submap_nodes,
      std::shared_ptr<voxblox::TsdfMap> submap, std::function<void()> work_item)
      REQUIRES(mutex_);
  ...
  void ConstructSubmapScanMatcher(
      const mapping::SubmapId& submap_id,
      const std::vector<mapping::TrajectoryNode>& submap_nodes,
      std::shared_ptr<voxblox::TsdfMap> submap) EXCLUDES(mutex_);
