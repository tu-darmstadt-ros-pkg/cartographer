...
#ifndef CARTOGRAPHER_MAPPING_3D_SPARSE_POSE_GRAPH_CONSTRAINT_BUILDER_VOXBLOX_ESDF_CONVERSION_H_
#define CARTOGRAPHER_MAPPING_3D_SPARSE_POSE_GRAPH_CONSTRAINT_BUILDER_VOXBLOX_ESDF_CONVERSION_H_
...
#include "cartographer/mapping_3d/voxblox_esdfs.h"
...
class ConstraintBuilderVoxbloxESDFConversion {
 public:
  using Constraint = mapping::SparsePoseGraph::Constraint;
  using Result = std::vector<Constraint>;

  ConstraintBuilderVoxbloxESDFConversion(
      const mapping::sparse_pose_graph::proto::ConstraintBuilderOptions&
          options,
      common::ThreadPool* thread_pool);
  ~ConstraintBuilderVoxbloxESDFConversion();

  ConstraintBuilderVoxbloxESDFConversion(const ConstraintBuilderVoxbloxESDFConversion&) = delete;
  ConstraintBuilderVoxbloxESDFConversion& operator=(const ConstraintBuilderVoxbloxESDFConversion&) = delete;
  ...
  void MaybeAddConstraint(const mapping::SubmapId& submap_id, const VoxbloxESDF* const submap,
      const mapping::NodeId& node_id, const sensor::CompressedPointCloud * const compressed_point_cloud,
  ...
  void MaybeAddGlobalConstraint(
      const mapping::SubmapId& submap_id, const VoxbloxESDF* const submap,
 ...
 private:
  struct SubmapScanMatcher {
    std::shared_ptr<voxblox::EsdfMap> hybrid_grid;
  ...
  void ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(
      ...
      const std::vector<mapping::TrajectoryNode>& submap_nodes,
      std::shared_ptr<voxblox::EsdfMap> submap, std::function<void()> work_item)
      REQUIRES(mutex_);
  ...
  void ConstructSubmapScanMatcher(
      const mapping::SubmapId& submap_id,
      const std::vector<mapping::TrajectoryNode>& submap_nodes,
      std::shared_ptr<voxblox::EsdfMap> submap) EXCLUDES(mutex_);

  // Returns the scan matcher for a submap, which has to exist.
  const SubmapScanMatcher* GetSubmapScanMatcher(const mapping::SubmapId& submap_id)
      EXCLUDES(mutex_);
  ...
  void ComputeConstraint(
          ...
          const sensor::CompressedPointCloud* const compressed_point_cloud,
          const transform::Rigid3d& initial_pose,
          std::unique_ptr<OptimizationProblem::Constraint>* constraint) EXCLUDES(mutex_);
...
#endif  // CARTOGRAPHER_MAPPING_3D_SPARSE_POSE_GRAPH_CONSTRAINT_BUILDER_VOXBLOX_CONVERSION_H_
