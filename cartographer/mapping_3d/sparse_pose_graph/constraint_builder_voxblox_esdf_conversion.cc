...
#include "cartographer/mapping_3d/sparse_pose_graph/constraint_builder_voxblox_esdf_conversion.h"
...
ConstraintBuilderVoxbloxESDFConversion::ConstraintBuilderVoxbloxESDFConversion(
...
void ConstraintBuilderVoxbloxESDFConversion::MaybeAddConstraint(const mapping::SubmapId& submap_id, const VoxbloxESDF * const submap,
    ...
    if (sampler_.Pulse()) {
      ...
      ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(
          submap_id, submap_nodes, submap->esdf,
          [=]() EXCLUDES(mutex_) {
...
void ConstraintBuilderVoxbloxESDFConversion::MaybeAddGlobalConstraint(const mapping::SubmapId& submap_id, const VoxbloxESDF * const submap,
        const mapping::NodeId& node_id,
        const sensor::CompressedPointCloud* const compressed_point_cloud,
        const std::vector<mapping::TrajectoryNode>& submap_nodes,
...
void ConstraintBuilderVoxbloxESDFConversion::NotifyEndOfScan() {
  common::MutexLocker locker(&mutex_);
  ++current_computation_;
}

void ConstraintBuilderVoxbloxESDFConversion::WhenDone(
    const std::function<void(const ConstraintBuilderVoxbloxESDFConversion::Result&)> callback) {
  common::MutexLocker locker(&mutex_);
  CHECK(when_done_ == nullptr);
  when_done_ =
...
void ConstraintBuilderVoxbloxESDFConversion::ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(
    const mapping::SubmapId& submap_id,
    const std::vector<mapping::TrajectoryNode>& submap_nodes,
    std::shared_ptr<voxblox::EsdfMap> submap, const std::function<void()> work_item) {
...
void ConstraintBuilderVoxbloxESDFConversion::ConstructSubmapScanMatcher(
    const mapping::SubmapId& submap_id,
    const std::vector<mapping::TrajectoryNode>& submap_nodes,
    std::shared_ptr<voxblox::EsdfMap> submap) {
...
const ConstraintBuilderVoxbloxESDFConversion::SubmapScanMatcher*
ConstraintBuilderVoxbloxESDFConversion::GetSubmapScanMatcher(const mapping::SubmapId& submap_id) {
common::MutexLocker locker(&mutex_);
...
void ConstraintBuilderVoxbloxESDFConversion::ComputeConstraint(
    const mapping::SubmapId& submap_id, const VoxbloxESDF* const submap, const mapping::NodeId& node_id,bool match_full_submap,
...
void ConstraintBuilderVoxbloxESDFConversion::FinishComputation(const int computation_index) {
  Result result;
...
int ConstraintBuilderVoxbloxESDFConversion::GetNumFinishedScans() {
  common::MutexLocker locker(&mutex_);
  if (pending_computations_.empty()) {
