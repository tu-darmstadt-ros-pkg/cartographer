...
std::vector<mapping::TrajectoryNode> ComputeSubmapNodes(
    ...
    const TSDF* const submap, int scan_index,
...
ConstraintBuilderTSDF::ConstraintBuilderTSDF(
...
void ConstraintBuilderTSDF::MaybeAddConstraint(
    const int submap_index, const TSDF* const submap, const int scan_index,
    const std::vector<mapping::TrajectoryNode>& trajectory_nodes,
    const transform::Rigid3d& initial_relative_pose) {
  ...
  if (sampler_.Pulse()) {
    ...
    ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(
        submap_index, submap_nodes, submap->tsdf,
        [=]() EXCLUDES(mutex_) {
...
void ConstraintBuilderTSDF::MaybeAddGlobalConstraint(
    const int submap_index, const TSDF* const submap, const int scan_index,
...
void ConstraintBuilderTSDF::NotifyEndOfScan(const int scan_index) {
  common::MutexLocker locker(&mutex_);
  CHECK_EQ(current_computation_, scan_index);
  ++current_computation_;
}

void ConstraintBuilderTSDF::WhenDone(
    const std::function<void(const ConstraintBuilderTSDF::Result&)> callback) {
  common::MutexLocker locker(&mutex_);
  CHECK(when_done_ == nullptr);
  when_done_ =
...
void ConstraintBuilderTSDF::ScheduleSubmapScanMatcherConstructionAndQueueWorkItem(
    const int submap_index,
    const std::vector<mapping::TrajectoryNode>& submap_nodes,
    chisel::ChiselConstPtr<chisel::DistVoxel> submap, const std::function<void()> work_item) {
  if (submap_scan_matchers_[submap_index].fast_correlative_scan_matcher !=
    ...
    if (submap_queued_work_items_[submap_index].size() == 1) {
      thread_pool_->Schedule(
          std::bind(std::mem_fn(&ConstraintBuilderTSDF::ConstructSubmapScanMatcher),
                    this, submap_index, submap_nodes, submap));
    }
  }
}

void ConstraintBuilderTSDF::ConstructSubmapScanMatcher(
    const int submap_index,
    const std::vector<mapping::TrajectoryNode>& submap_nodes,
    chisel::ChiselConstPtr<chisel::DistVoxel> submap) {
  auto submap_scan_matcher =
      common::make_unique<scan_matching::FastCorrelativeTSDFScanMatcher>(
          submap, submap_nodes,
...
const ConstraintBuilderTSDF::SubmapScanMatcher*
ConstraintBuilderTSDF::GetSubmapScanMatcher(const int submap_index) {
  common::MutexLocker locker(&mutex_);
...
void ConstraintBuilderTSDF::ComputeConstraint(
    const int submap_index, const TSDF* const submap, const int scan_index,
    const mapping::Submaps* scan_trajectory,
...
void ConstraintBuilderTSDF::FinishComputation(const int computation_index) {
  Result result;
...
int ConstraintBuilderTSDF::GetNumFinishedScans() {
  common::MutexLocker locker(&mutex_);
  if (pending_computations_.empty()) {
