...
GlobalVoxbloxESDFTrajectoryBuilder::GlobalVoxbloxESDFTrajectoryBuilder(
        const proto::LocalTrajectoryBuilderOptions& options,
        const int trajectory_id, SparsePoseGraphVoxbloxESDFConversion* sparse_pose_graph)
    : trajectory_id_(trajectory_id),
      sparse_pose_graph_(sparse_pose_graph),
      local_trajectory_builder_(CreateLocalVoxbloxESDFTrajectoryBuilder(options)) {}

GlobalVoxbloxESDFTrajectoryBuilder::~GlobalVoxbloxESDFTrajectoryBuilder() {}

const mapping_3d::VoxbloxESDFs* GlobalVoxbloxESDFTrajectoryBuilder::submaps() const {
  return local_trajectory_builder_->submaps();
}

void GlobalVoxbloxESDFTrajectoryBuilder::AddImuData(
...
void GlobalVoxbloxESDFTrajectoryBuilder::AddRangefinderData(
...
void GlobalVoxbloxESDFTrajectoryBuilder::AddOdometerData(const common::Time time,
                                              const transform::Rigid3d& pose) {
  local_trajectory_builder_->AddOdometerData(time, pose);
}

const GlobalVoxbloxESDFTrajectoryBuilder::PoseEstimate&
GlobalVoxbloxESDFTrajectoryBuilder::pose_estimate() const {
