...
VoxbloxESDF::VoxbloxESDF(const float high_resolution, const float low_resolution,
...
VoxbloxESDFs::VoxbloxESDFs(const proto::TSDFsOptions& options)
    : options_(options) {
  // We always want to have at least one tsdf which we can return,
  // and will create it at the origin in absence of a better choice.
  AddESDF(transform::Rigid3d::Identity());
...
const std::shared_ptr<voxblox::TsdfIntegratorBase> VoxbloxESDFs::GetIntegrator(int index) const{
    CHECK_GE(index, 0);
    CHECK_LT(index, size());
...
std::vector<int> VoxbloxESDFs::insertion_indices() const {
  if (size() > 1) {
    return {size() - 2, size() - 1};
  }
  return {size() - 1};
}
void VoxbloxESDFs::InsertRangeData(const sensor::RangeData& range_data_in_tracking,
...
string VoxbloxESDFs::ComputePixelValues(
    const std::vector<VoxbloxESDFs::PixelData>& accumulated_pixel_data) const {
  string cell_data;
...
std::vector<VoxbloxESDFs::PixelData> VoxbloxESDFs::AccumulatePixelData(
...
void VoxbloxESDFs::SubmapToProto(
...
//  double truncation_distance = options_.projection_integrator_options().truncation_distance();
 ...
 // float max_truncation_distance = 0.15;//std::ceil(2.0 + (truncation_scale * truncation_distance /
  ...
  submaps_.emplace_back(new VoxbloxESDF(options_.high_resolution(), options_.low_resolution(), origin,
  ...
  esdf_integrator.reset(new voxblox::EsdfIntegrator(esdf_integrator_config,
                                            submaps_[size()-1]->tsdf->getTsdfLayerPtr(),
                                            submaps_[size()-1]->esdf->getEsdfLayerPtr()));
  esdf_integrators_.emplace_back(esdf_integrator);
