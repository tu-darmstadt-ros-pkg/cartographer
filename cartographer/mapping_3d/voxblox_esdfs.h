...
#ifndef CARTOGRAPHER_MAPPING_3D_VOXBLOX_ESDFS_H_
#define CARTOGRAPHER_MAPPING_3D_VOXBLOX_ESDFS_H_
...
struct VoxbloxESDF : public mapping::Submap {
  VoxbloxESDF(float high_resolution, float low_resolution,
...
class VoxbloxESDFs : public mapping::Submaps {
 public:
  VoxbloxESDFs();
  VoxbloxESDFs(const proto::TSDFsOptions& options);

  VoxbloxESDFs(const VoxbloxESDFs&) = delete;
  VoxbloxESDFs& operator=(const VoxbloxESDFs&) = delete;
 ...
 private:
  ...
  std::vector<std::unique_ptr<VoxbloxESDF>> submaps_;
  std::vector<std::shared_ptr<voxblox::TsdfIntegratorBase>> projection_integrators_;
  std::vector<std::shared_ptr<voxblox::EsdfIntegrator>> esdf_integrators_;
...
#endif  // CARTOGRAPHER_MAPPING_3D_VOXBLOX_ESDFS_H_
