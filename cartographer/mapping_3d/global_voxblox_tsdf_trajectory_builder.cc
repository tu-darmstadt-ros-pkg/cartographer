...
GlobalVoxbloxTSDFTrajectoryBuilder::GlobalVoxbloxTSDFTrajectoryBuilder(
        const proto::LocalTrajectoryBuilderOptions& options,
        const int trajectory_id, SparsePoseGraphVoxbloxConversion* sparse_pose_graph)
    : trajectory_id_(trajectory_id),
      sparse_pose_graph_(sparse_pose_graph),
      local_trajectory_builder_(CreateLocalVoxbloxTSDFTrajectoryBuilder(options)) {}

GlobalVoxbloxTSDFTrajectoryBuilder::~GlobalVoxbloxTSDFTrajectoryBuilder() {}

const mapping_3d::VoxbloxTSDFs* GlobalVoxbloxTSDFTrajectoryBuilder::submaps() const {
  return local_trajectory_builder_->submaps();
}

void GlobalVoxbloxTSDFTrajectoryBuilder::AddImuData(
...
void GlobalVoxbloxTSDFTrajectoryBuilder::AddRangefinderData(
...
void GlobalVoxbloxTSDFTrajectoryBuilder::AddOdometerData(const common::Time time,
                                              const transform::Rigid3d& pose) {
  local_trajectory_builder_->AddOdometerData(time, pose);
}

const GlobalVoxbloxTSDFTrajectoryBuilder::PoseEstimate&
GlobalVoxbloxTSDFTrajectoryBuilder::pose_estimate() const {
